...
export async function checkMCPHealth(): Promise<{
  ...
  console.log(
    `[MCP Client] Checking health of MCP service at ${MCP_SERVICE_URL}/health`,
  )
...
export async function connectMCP(
  ...
  console.log(
    `[MCP Client] Connecting to server '${config.name}' with session ID: ${sessionId}`,
  )
  ...
  if (error || !data) {
    console.error(
      `[MCP Client] Connection failed for session ${sessionId}:`,
      error,
    )
  ...
  if (connection.message === "Already connected") {
    console.log(
      `[MCP Client] Session ${sessionId} is already connected to server '${config.name}'`,
    )
  } else {
    console.log(
      `[MCP Client] Successfully connected session ${sessionId} to server '${config.name}'`,
    )
  }
...
export async function disconnectMCP(
  ...
  if (error || !data) {
    console.error(
      `[MCP Client] Disconnect failed for session ${sessionId}:`,
      error,
    )
...
export async function sendChatMCP(
  sessionId: string | null,
  modelId: string,
  ...
  console.log(
    `[MCP Client] Sending chat to model ${modelId} for session ${sessionId}`,
  )
  ...
  if (error || !data) {
    console.error(
      `[MCP Client] Chat request failed for session ${sessionId}:`,
      error,
    )
...
export async function getAvailableMCPServers() {
  console.log(
    `[MCP Client] Fetching available servers from ${MCP_SERVICE_URL}/servers`,
  )
