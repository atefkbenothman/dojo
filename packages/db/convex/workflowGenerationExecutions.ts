...
// Create a new workflow generation execution
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!userId) {
      throw new Error("Must be authenticated to create workflow generations")
    }

    const executionId = await ctx.db.insert("workflowGenerationExecutions", {
      userId,
      prompt: args.prompt,
...
export const updateStatus = mutation({
  args: {
    executionId: v.id("workflowGenerationExecutions"),
    status: v.union(
      v.literal("running"),
      v.literal("completed"),
      v.literal("failed"),
...
export const getByUser = query({
  ...
  handler: async (ctx, args) => {
    ...
    const userId = await getCurrentUserId(ctx)
    if (!userId) {
      throw new Error("Must be authenticated to get workflow generations")
    }

    let query = ctx.db.query("workflowGenerationExecutions").withIndex("by_user", (q) => q.eq("userId", userId))
...
export const getActiveExecution = query({
  ...
  handler: async (ctx, args) => {
    ...
    const execution = await ctx.db
      .query("workflowGenerationExecutions")
